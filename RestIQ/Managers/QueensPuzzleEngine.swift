...
//  RestIQ
//
//  Fixed: private helper types (Solver, RegionGenerator, SeededRandomNumberGenerator)
//  marked nonisolated to opt out of the project-wide default MainActor isolation.
//
...
public actor QueensPuzzleEngine {
    ...
    public nonisolated static func generate(
        ...
        if let seed {
            ...
            if let engine = tryGenerate(rng: &rng, maxTries: attempts) {
                return engine
            }
            var fallback = SeededRandomNumberGenerator(seed: seed &+ 0x9e3779b97f4a7c15)
            if let engine = tryGenerate(rng: &fallback, maxTries: attempts / 2) {
                return engine
            }
            return nil
        } else {
            ...
            for _ in fallbackDifficulties {
                var altRng = SystemRandomNumberGenerator()
                if let engine = tryGenerate(rng: &altRng, maxTries: attempts / 2) {
                    return engine
                }
            }
            return nil
...
// MARK: - Solver
// Explicitly nonisolated to opt out of project-wide @MainActor default isolation.
        ...
        for r in 0..<size {
            for c in 0..<size where map[r][c] == -1 {
                ...
                for d in directions {
                    ...
                    if nr >= 0, nr < size, nc >= 0, nc < size, map[nr][nc] != -1 {
                        map[r][c] = map[nr][nc]; assigned = true; break
                    }
                }
